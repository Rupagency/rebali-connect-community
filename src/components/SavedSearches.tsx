...
export default function SavedSearches() {
  ...
  const handleAdd = async () => {
    ...
    if (searches.length >= maxSearches) {
      toast({ title: (t('savedSearches.maxReached') || '').replace('{{max}}', String(maxSearches)), variant: 'destructive' });
      return;
    }
    ...
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center gap-2">
          ...
          <Badge variant="secondary" className="ml-auto">{searches.length}/{maxLabel}</Badge>
        </CardTitle>
        <CardDescription>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="flex gap-2">
          ...
          <Button
            onClick={handleAdd}
            disabled={adding || searches.length >= maxSearches || !newKeyword.trim()}
            size="sm"
            className="gap-1"
          >
